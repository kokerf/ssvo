...
namespace ceres_slover {
    ...
    public:
        ...
        PreintegrationRotationError(const Preintegration* preint) : preint_(preint) {}
        ...
        virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
        {
            ...
            Vector3d tangent_dRbg = preint_->jacobdRBiasGyro() * delta_biasgyro;
            const Matrix3d dRbg = Sophus::SO3d::exp(tangent_dRbg).matrix();
            const Matrix3d Rij = Rwi.matrix().transpose() * Rwj.matrix();
            const Matrix3d res_dRij = (preint_->deltaRij() * dRbg).transpose() * Rij;
            res = Sophus::SO3d::log(res_dRij);
            ...
            if (nullptr != jacobian2)
            {
                ...
                jacob_dR_dbiasgyro = -left_jacob_of_res * right_jacob_of_dR_dbiasgyro * preint_->jacobdRBiasGyro();
            }
            return true;
        }

        static inline ceres::CostFunction *Create(const Preintegration* preint)
        { return (new PreintegrationRotationError(preint)); }

    private:

        const Preintegration * preint_;
